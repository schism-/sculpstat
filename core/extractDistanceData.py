...
class DistanceData(object):
    ...
    def __init__(self, model_name, max_step, output_folder):
    ...
    def extract_data(self):
        for k in range(1, self.max_step):
            ...
            self.final_data[k] = temp

        fh_final = open(self.output_folder + self.model_name + "/distance_data.txt", "w")
        for step in self.final_data:
            fh_final.write(str(step) + "\n")
            for idx, value in self.final_data[step]:
                fh_final.write("%d %.10f " % (idx, value))
            fh_final.write("\n")
        fh_final.close()
    ...
    def generate_script(self, diff_no):
        ...
        file_content = "<!DOCTYPE FilterScript>\n"  + \
                       ...
                       "<Param type=\"RichInt\"     name=\"SampleNum\"      value=\"" + str(vertices_no) + "\" />\n"  + \
